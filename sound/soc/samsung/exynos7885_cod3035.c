...
static int sma1303_startup(struct snd_pcm_substream *substream)
{
...
static void sma1303_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_card *card = rtd->card;
...
static const struct snd_soc_ops uaif3_ops = {
	.startup = sma1303_startup,
	.shutdown = sma1303_shutdown,
};
...
{
	struct snd_soc_pcm_runtime *rtd;
	struct snd_soc_dai *aif1_dai, *codec_dai;
	...
	register_cod3035_jack_cb(codec);
