...
#else
	if (rdc_string[0] != 0) {
		strlcpy(cal_done_string, rdc_string, FILESIZE_RDC_CAL);
		ret = 0;
		goto tfa_cal_rdc_show_exit;
	}

	pr_info("%s: failed to read stored value, and get from amplifier\n",
		__func__);
#endif
...
#else
	if (temp_string[0] != 0) {
		strlcpy(cal_done_string, temp_string, FILESIZE_TEMP_CAL);
		ret2 = 0;
		goto tfa_cal_temp_show_exit;
	}

	pr_info("%s: failed to read stored value, and get from driver\n",
		__func__);
#endif
