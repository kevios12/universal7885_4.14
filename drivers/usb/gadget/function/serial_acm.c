...
static long modem_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret;
	ret = modem_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
	return ret;
}
#endif
...
int modem_register(void *data)
{
	...
	printk(KERN_INFO "DUN is registered\n");
