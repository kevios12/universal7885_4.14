...
int decon_wait_for_vsync(struct decon_device *decon, u32 timeout)
{
	...
	if (decon->dt.psr_mode == DECON_MIPI_COMMAND_MODE && atomic_read(&decon->bypass))
		return 0;
...
static void __decon_update_regs(struct decon_device *decon, struct decon_reg_data *regs)
{
	...
	if (decon_reg_start(decon->id, &psr) < 0 && !atomic_read(&decon->bypass)) {
		decon_dump(decon);
#ifdef CONFIG_LOGGING_BIGDATA_BUG
...
//				decon->timeline->value, decon->timeline_max);
		...
		decon_wait_for_vsync(decon, VSYNC_TIMEOUT_MSEC);
		decon_wait_for_vstatus(decon, 50);
		if (decon_reg_wait_for_update_timeout(decon->id, SHADOW_UPDATE_TIMEOUT) < 0 && !atomic_read(&decon->bypass)) {
			decon_dump(decon);
#ifdef CONFIG_LOGGING_BIGDATA_BUG
			log_decon_bigdata(decon);
#endif
