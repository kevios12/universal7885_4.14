...
#include "hx83102d_a10eu_param.h"
#include "dd.h"
...
#endif
	if (cpu_to_be32(lcd->id_info.value) == 0x9A6234) {
		dev_info(&lcd->ld->dev, "%s: 2nd panel.\n", __func__);
		...
		dev_info(&lcd->ld->dev, "%s: 1st panel.\n", __func__);
				...
				unsigned long event, void *data)
{
	...
	if (fb_blank == FB_BLANK_UNBLANK) {
		...
		run_list(lcd->dsim->dev, "dsim_set_panel_power_disable_late");
	}
...
static int hx83102d_probe(struct lcd_info *lcd)
{
	...
	lm36274_i2c_id->driver_data = (kernel_ulong_t)lcd;
