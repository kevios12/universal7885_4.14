...
#define m2m1shot2_fence_put(fence) do { dma_fence_put(fence); fence = NULL; } while (0)

static void m2m1shot2_fence_callback(struct dma_fence *fence,
					struct dma_fence_cb *cb);
struct dma_fence *m2m1shot2_get_acquire_fence(struct m2m1shot2_context *ctx,
                                        struct m2m1shot2_context_image *img, s32 fence_fd);
static const char *m2m1shot2_fence_get_driver_name(struct dma_fence *fence);
static bool m2m1shot2_fence_enable_signaling(struct dma_fence *fence);
static void m2m1shot2_fence_release(struct dma_fence *fence);
static void m2m1shot2_fence_value_str(struct dma_fence *fence, char *str, int size);

static struct dma_fence_ops m2m1shot2_fence_ops = {
        .get_driver_name =      m2m1shot2_fence_get_driver_name,
        .get_timeline_name =    m2m1shot2_fence_get_driver_name,
        .enable_signaling =     m2m1shot2_fence_enable_signaling,
        .wait =                 dma_fence_default_wait,
        .release =              m2m1shot2_fence_release,
        .fence_value_str =      m2m1shot2_fence_value_str,
};
...
static int m2m1shot2_open(struct inode *inode, struct file *filp)
{
	...
	spin_lock_init(&m21dev->fence_timeout_lock);
	spin_lock_init(&m21dev->fence_lock);
				...
				struct m2m1shot2_context_image *img)
{
	...
	if (!!(img->flags & M2M1SHOT2_IMGFLAG_ACQUIRE_FENCE) && img->fence) {
		dma_fence_remove_callback(img->fence, &img->fence_cb);
		m2m1shot2_fence_put(img->fence);
					...
					bool success)
{
	...
	if (ctx->release_fence) {
		dma_fence_signal(ctx->release_fence->fence);
		dma_fence_put(ctx->release_fence->fence);
		fput(ctx->release_fence->file);
	}

	for (i = 0; i < ctx->num_sources; i++) {
		m2m1shot2_fence_put(image[i].img.fence);
		image[i].img.flags &= ~M2M1SHOT2_IMGFLAG_ACQUIRE_FENCE;
	}

	m2m1shot2_fence_put(ctx->target.fence);
...
#define is_vma_cached(vma)						      \
	((pgprot_val(pgprot_noncached((vma)->vm_page_prot)) != pgprot_val((vma)->vm_page_prot)) &&    \
	   (pgprot_val(pgprot_writecombine((vma)->vm_page_prot)) != pgprot_val((vma)->vm_page_prot)))
				...
				bool skip_inv)
{
	...
	if (img->memory == M2M1SHOT2_BUFTYPE_USERPTR) {
		...
		for (i = 0; i < img->num_planes; i++) {
			if (inv)
				dma_sync_sg_for_cpu(m21dev->dev,
						img->plane[i].sgt->sgl,
						img->plane[i].sgt->orig_nents, dir);
				      ...
				      enum dma_data_direction dir)
{
	...
	for (i = 0; i < img->num_planes; i++) {
		if (m2m1shot2_need_shareable_flush(
			...
			dma_sync_sg_for_cpu(ctx->m21dev->dev,
				img->plane[i].sgt->sgl,
				img->plane[i].sgt->orig_nents, dir);
		else
			dma_sync_sg_for_device(ctx->m21dev->dev,
				img->plane[i].sgt->sgl,
				img->plane[i].sgt->orig_nents, dir);
	}
}
					...
					struct dma_fence_cb *cb)
{
	// cb, fence_cb
	struct m2m1shot2_context_image *img =
		container_of(cb, struct m2m1shot2_context_image, fence_cb);
	...
	spin_lock_irqsave(&ctx->m21dev->fence_timeout_lock, flags);
	kref_put(&ctx->starter, m2m1shot2_schedule_queuework);
	spin_unlock_irqrestore(&ctx->m21dev->fence_timeout_lock, flags);
}
...
static void m2m1shot2_timeout_handler(unsigned long arg)
{
	...
	pr_err("%s: %d Fence(s) timed out after %d msec.\n", __func__,
		atomic_read(&ctx->starter.refcount.refs), M2M1S2_TIMEOUT_INTERVAL);

	for (i = 0; i < ctx->num_sources; i++) {
		fence = image[i].img.fence;
		if (fence) {
			strlcpy(name, fence->ops->get_driver_name(fence), sizeof(name));
			pr_err("%s:    SOURCE[%d]: %s #%d (%s)\n",
				__func__, i, name, fence->seqno,
				dma_fence_is_signaled(fence) ? "signalled" : "active");
		}
	}

	fence = ctx->target.fence;
	if (fence) {
                strlcpy(name, fence->ops->get_driver_name(fence), sizeof(name));
	...
	if (ctx->release_fence->fence)
		pr_err("%s:    Pending release fence: #%d\n",
			__func__, ctx->release_fence->fence->seqno);
	 ...
	 * signal.
	*/
	if (atomic_read(&ctx->starter.refcount.refs) == 0) {
		spin_unlock_irqrestore(&ctx->m21dev->fence_timeout_lock, flags);
	...
	for (i = 0; i < ctx->num_sources; i++) {
		fence = image[i].img.fence;
		if (fence)
			dma_fence_remove_callback(fence, &image[i].img.fence_cb);
	}

	fence = ctx->target.fence;
	if (fence)
		dma_fence_remove_callback(fence, &ctx->target.fence_cb);
				    ...
				    struct m2m1shot2_context *ctx)
{
	 ...
	 * task
	 */
	refcount = atomic_read(&ctx->starter.refcount.refs);
	if (refcount > 1)
		mod_timer(&ctx->timer,
			jiffies + msecs_to_jiffies(M2M1S2_TIMEOUT_INTERVAL));
				...
				struct m2m1shot2_image *src)
{
	...
	//The acquire fence is set here before the buffer data is accessed.
	img->img.fence = m2m1shot2_get_acquire_fence(ctx, &img->img, src->fence);
				...
				struct m2m1shot2_image *dst)
{
	...
	//The acquire fence for the target should be set here.
	//Before the buffer data is accessed.
	img->fence = m2m1shot2_get_acquire_fence(ctx, img, dst->fence);
					...
					unsigned int num_sources)
{
	...
	struct m2m1shot2_context_image *img;
	unsigned int i;
	s32 release_fences[M2M1SHOT2_MAX_IMAGES + 1];
	s32 target_release_fence;
	...
	dma_fence_init(fence, &m2m1shot2_fence_ops, &ctx->m21dev->fence_lock,
		ctx->fence_context,
		atomic_inc_return(&ctx->fence_timeline));
	...
	for(i = 0; i < ctx->num_sources; i++) {
		img = &ctx->source[i].img;
		if(!!(img->flags & M2M1SHOT2_IMGFLAG_RELEASE_FENCE)) {
			if(copy_to_user(&usersrc[i].fence, &release_fences[i], sizeof(s32))) {
				ret = -EFAULT;
				dev_err(ctx->m21dev->dev, "%s: Release fence set fail for source %d\n",
                                	__func__, release_fences[i]);
				goto err;
			}
		}
	}
	...
	img = &ctx->target;
	if (!!(ctx->target.flags & M2M1SHOT2_IMGFLAG_RELEASE_FENCE)) {
		target_release_fence = get_unused_fd_flags(O_CLOEXEC);
                if (target_release_fence < 0)
                        goto err;
                fd_install(target_release_fence, get_file(file->file));

		if(copy_to_user(&usertgt->fence, &target_release_fence, sizeof(s32))) {
			ret = -EFAULT;
			dev_err(ctx->m21dev->dev, "%s: Release fence set fail for target %d\n",
        	                        __func__, target_release_fence);
			goto err;
		}
	}

	ctx->release_fence = file;
                                        ...
                                        struct m2m1shot2_context_image *img, s32 fence_fd)
{
	...
	unsigned long flags;
		...
		/*dev_warn(dev, "%s: Acquire fence not set by libacryl %d\n",
				__func__, fence_fd);*/
                return NULL;
	}
        ...
        if (!fence) {
		dev_err(dev, "%s:: Failed to get acquire fence %d\n",
                                __func__, fence);
                return NULL;
	}

	for (i = 0; i < ctx->num_sources; i++) {
		if ((ctx->source[i].img.fence != NULL) && (ctx->source[i].img.fence == fence))
                        return NULL;
        }
	...
	if(dma_fence_get_status(fence) != 0) {
        ...
        ret = dma_fence_add_callback(fence, &img->fence_cb, m2m1shot2_fence_callback);
        if (ret < 0) {
		dev_err(dev, "%s: Failed to add fence callback!", __func__);
                kref_put(&ctx->starter, m2m1shot2_context_schedule_start);
                dma_fence_put(fence);
                return NULL;
        }
