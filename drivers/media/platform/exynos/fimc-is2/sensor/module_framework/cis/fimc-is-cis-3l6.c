...
int sensor_3l6_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	...
	if (ret < 0)
		 goto p_err;
	ret = fimc_is_sensor_write16(client, SENSOR_3L6_Y_OUTPUT_SIZE_ADDR, cis_data->cur_height);
	if (ret < 0)
		 goto p_err;
	...
	if (ret < 0)
		goto p_err;
	ret = fimc_is_sensor_write8(client, SENSOR_3L6_BINNING_TYPE_ADDR, (ratio_w << 4) | ratio_h);
	if (ret < 0)
		goto p_err;
