...
struct fimc_is_clk fimc_is_clk_list[] = {
	...
	REGISTER_CLK("GATE_IS_GDC"),
	REGISTER_CLK("GATE_SMMU_IS0"),
	REGISTER_CLK("GATE_SMMU_IS1"),

	REGISTER_CLK("GATE_CAM_3AA1"),
	REGISTER_CLK("GATE_CAM_3AA0"),
	REGISTER_CLK("GATE_CAM_CSIS_DMA"),
	REGISTER_CLK("GATE_CAM_CSIS0"),
	REGISTER_CLK("GATE_CAM_CSIS1"),
	REGISTER_CLK("GATE_CAM_CSIS2"),
	REGISTER_CLK("GATE_CAM_CSIS3"),
	REGISTER_CLK("GATE_CAM_SMMU"),
...
static void exynos7885_fimc_is_print_clk_reg(void)
{

}

static int exynos7885_fimc_is_print_clk(struct device *dev)
{
	fimc_is_get_rate(dev, "GATE_IS_ISP");
	fimc_is_get_rate(dev, "GATE_IS_MCSC");
	fimc_is_get_rate(dev, "GATE_IS_VRA");
	fimc_is_get_rate(dev, "GATE_IS_GDC");
	fimc_is_get_rate(dev, "GATE_SMMU_IS0");
	fimc_is_get_rate(dev, "GATE_SMMU_IS1");

	fimc_is_get_rate(dev, "GATE_CAM_3AA1");
	fimc_is_get_rate(dev, "GATE_CAM_3AA0");
	fimc_is_get_rate(dev, "GATE_CAM_CSIS_DMA");
	fimc_is_get_rate(dev, "GATE_CAM_CSIS0");
	fimc_is_get_rate(dev, "GATE_CAM_CSIS1");
	fimc_is_get_rate(dev, "GATE_CAM_CSIS2");
	fimc_is_get_rate(dev, "GATE_CAM_CSIS3");
	fimc_is_get_rate(dev, "GATE_CAM_SMMU");

	fimc_is_get_rate(dev, "CIS_CLK0");
	fimc_is_get_rate(dev, "CIS_CLK1");
	fimc_is_get_rate(dev, "CIS_CLK2");

	exynos7885_fimc_is_print_clk_reg();

	return 0;
}

int exynos7885_fimc_is_clk_gate(u32 clk_gate_id, bool is_on)
{
	return 0;
}

int exynos7885_fimc_is_uart_gate(struct device *dev, bool mask)
{
	return 0;
}

int exynos7885_fimc_is_get_clk(struct device *dev)
{
	struct clk *clk;
	int i;
...
int exynos7885_fimc_is_cfg_clk(struct device *dev)
{
	return 0;
}

int exynos7885_fimc_is_clk_on(struct device *dev)
{
	int ret = 0;
	...
	fimc_is_enable(dev, "GATE_CAM_3AA1");
	fimc_is_enable(dev, "GATE_CAM_3AA0");

	if (debug_clk > 1)
		exynos7885_fimc_is_print_clk(dev);
...
int exynos7885_fimc_is_clk_off(struct device *dev)
{
	int ret = 0;
	struct exynos_platform_fimc_is *pdata;
	...
	fimc_is_disable(dev, "GATE_CAM_3AA1");
	fimc_is_disable(dev, "GATE_CAM_3AA0");
...
{
	exynos7885_fimc_is_clk_gate(clk_gate_id, is_on);
	return 0;
}
