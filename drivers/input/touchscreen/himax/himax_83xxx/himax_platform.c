			...
			struct himax_i2c_platform_data *pdata)
{
	...
	count = of_property_count_strings(dt, "himax,fw-path");
	if (count <= 0) {
		pdata->i_CTPM_firmware_name = NULL;
	} else {
		u8 lcd_id_num = of_property_count_u32_elems(dt, "himax,select_lcdid");

		if ((lcd_id_num != count) || (lcd_id_num <= 0)) {
			of_property_read_string_index(dt, "himax,fw-path", 0, &pdata->i_CTPM_firmware_name);
		} else {
			...
			for (i = 0; i < lcd_id_num; i++) {
				if (lcd_id[i] == lcdtype) {
					of_property_read_string_index(dt, "himax,fw-path", i, &pdata->i_CTPM_firmware_name);
					break;
				}
			}
		}
	...
	input_info(true, &ts->client->dev, "%s DT: lcdtype %X, fw %s \n",
			HIMAX_LOG_TAG, lcdtype, pdata->i_CTPM_firmware_name);
#else
...
#if defined(CONFIG_EXYNOS_DECON_FB)
	input_info(true, &client->dev, "%s %s: lcdtype %X\n", HIMAX_LOG_TAG, __func__, lcdtype);
