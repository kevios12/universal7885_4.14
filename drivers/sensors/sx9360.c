...
static int sx9360_parse_dt(struct sx9360_p *data, struct device *dev)
{
	...
	if (!sx9360_read_setupreg(dNode, "sx9360,reggnrlctrl2", &reggnrlctrl2))
		setup_reg[SX9360_GNRLCTRL_REG_IDX + 2].val = reggnrlctrl2;
	if (!sx9360_read_setupreg(dNode, "sx9360,regafeparam1phm", &regafeparam1phm))
	...
	if (!sx9360_read_setupreg(dNode, "sx9360,regproxctrl3", &regproxctrl3))
...
{
	PD_NOTI_ATTACH_TYPEDEF usb_typec_info =
