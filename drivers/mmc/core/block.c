...
#if defined(CONFIG_SEC_ABC)
			if ((err_log[index].hw_rst_cnt % 10) == 0)
...
clear_dcmd:
	...
	status = err ? BLK_STS_IOERR : BLK_STS_OK;
...
static void mmc_blk_cmdq_issue_drv_op(struct mmc_queue *mq, struct request *req) {
	...
	mq_rq = req->special;
	if (md->area_type & MMC_BLK_DATA_AREA_RPMB) {
		pr_debug("%s: CMDQ : try halt for access rpmb [drv_op : %d]\n", mmc_hostname(card->host), mq_rq->drv_op);
		...
		pr_debug("%s: CMDQ : try halt for legacy cmd [drv_op : %d]\n", mmc_hostname(card->host), mq_rq->drv_op);
		ret = mmc_cmdq_halt_on_empty_queue(card->host);
		if (ret) {
			pr_err("%s: halt failed while doing %s err (%d)\n",
					mmc_hostname(card->host), __func__,
...
out:
	if (md->area_type & MMC_BLK_DATA_AREA_RPMB) {
		pr_debug("%s: CMDQ : try unhalt for access rpmb [drv_op : %d]\n", mmc_hostname(card->host), mq_rq->drv_op);
		...
		host->cmdq_ops->enable(host);
		pr_debug("%s: CMDQ : try unhalt after legacy cmd\n", mmc_hostname(card->host));
		if (mmc_cmdq_halt(card->host, false))
			pr_err("%s: %s: cmdq unhalt failed\n",
				mmc_hostname(card->host), __func__);
	}
	...
	list_for_each_entry_safe_reverse(mrq_t, mrq_n, &mq->eh_mrq, eh_entry) {
		...
		if (mqrq_t->retries <= mqrq_t->allowed) {
			...
			pr_err("[CQ] %s:----- Finish request: tag %d, sector %lx, nr %u, retries %d\n",
					...
					blk_rq_sectors(mrq_t->req),
					mqrq_t->retries);
			blk_end_request_all(mrq_t->req, err ? BLK_STS_IOERR : BLK_STS_OK);
		}
		spin_lock_bh(&mq->eh_lock);
	}
	spin_unlock_bh(&mq->eh_lock);
...
void mmc_blk_cmdq_req_done(struct mmc_request *mrq)
{
	...
	/* clear pending request */
	BUG_ON(!test_and_clear_bit(req->tag, &ctx_info->active_reqs));
