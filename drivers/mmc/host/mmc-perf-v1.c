...
/* control knob */
static int __ctrl_dvfs(struct mmc_perf *perf, enum ctrl_op op)
{
	struct pm_qos_request *pm_qos_int = &perf->pm_qos_int;
	struct pm_qos_request *pm_qos_mif = &perf->pm_qos_mif;
	struct pm_qos_request *pm_qos_cluster0 = &perf->pm_qos_cluster0;
	struct pm_qos_request *pm_qos_cluster1 = &perf->pm_qos_cluster1;
	struct pm_qos_request *pm_qos_cluster2 = &perf->pm_qos_cluster2;
	s32 pm_qos_int_value = perf->pm_qos_int_value;
	s32 pm_qos_mif_value = perf->pm_qos_mif_value;
	s32 pm_qos_cluster0_value = perf->pm_qos_cluster0_value;
	s32 pm_qos_cluster1_value = perf->pm_qos_cluster1_value;
	s32 pm_qos_cluster2_value = perf->pm_qos_cluster2_value;

	trace_mmc_perf_lock("dvfs", op);
	if (op == CTRL_OP_UP) {
		if (perf->pm_qos_int_value)
			pm_qos_update_request(pm_qos_int, pm_qos_int_value);
		if (perf->pm_qos_mif_value)
			pm_qos_update_request(pm_qos_mif, pm_qos_mif_value);
		if (perf->pm_qos_cluster0_value)
			pm_qos_update_request(pm_qos_cluster0, pm_qos_cluster0_value);
		if (perf->pm_qos_cluster1_value)
			pm_qos_update_request(pm_qos_cluster1, pm_qos_cluster1_value);
		if (perf->pm_qos_cluster2_value)
			pm_qos_update_request(pm_qos_cluster2, pm_qos_cluster2_value);
	} else if (op == CTRL_OP_DOWN) {
		if (perf->pm_qos_int_value)
			pm_qos_update_request(pm_qos_int, 0);
		if (perf->pm_qos_mif_value)
			pm_qos_update_request(pm_qos_mif, 0);
		if (perf->pm_qos_cluster0_value)
			pm_qos_update_request(pm_qos_cluster0, 0);
		if (perf->pm_qos_cluster1_value)
			pm_qos_update_request(pm_qos_cluster1, 0);
		if (perf->pm_qos_cluster2_value)
			pm_qos_update_request(pm_qos_cluster2, 0);
	} else {
		return -1;
	}
...
static int __ctrl_affinity(struct mmc_perf *perf, enum ctrl_op op)
{
	if (perf->cmdq_pid < 0)
		return -1;

	trace_mmc_perf_lock("affinity", op);
	if (op == CTRL_OP_UP)
		sched_setaffinity(perf->cmdq_pid, &perf->big_cl_mask);
	else if (op == CTRL_OP_DOWN)
		sched_setaffinity(perf->cmdq_pid, &perf->small_cl_mask);
	else
		return -1;
...
/* policy */
static enum policy_res __policy_heavy(struct mmc_perf *perf, u32 i_policy)
{
	int index;
	enum ctrl_op op;
	unsigned long flags;
	struct mmc_perf_stat_v1 *stat = &perf->stat_v1;
...
static enum policy_res (*__policy_set[__POLICY_MAX])(struct mmc_perf *perf, u32 i_policy) = {
	__policy_heavy,
};

/* updater(stat) */
static void __transit_for_count(struct mmc_perf_stat_v1 *stat,
...
static void __transit_for_hat(struct mmc_perf_stat_v1 *stat,
			      enum __traffic traffic, s64 time)
{
	s64 diff;
	bool transit = false;
...
static void __update_v1_queued(struct mmc_perf_stat_v1 *stat, u32 qd)
{
	ktime_t time = ktime_get();
	enum __traffic traffic;
	s64 diff;
	...
	trace_mmc_perf_update_v1("queued", stat->count, stat->hat_state,
				 stat->freq_state,
				 ktime_to_us(ktime_sub(ktime_get(), time)));
}

static void __update_v1_reset(struct mmc_perf_stat_v1 *stat)
{
	mod_timer(&stat->reset_timer,
		  jiffies + msecs_to_jiffies(stat->th_reset_in_ms));
}

static enum policy_res __do_policy(struct mmc_perf *perf)
{
	struct mmc_perf_stat_v1 *stat = &perf->stat_v1;
	enum policy_res res = R_OK;
...
static enum policy_res __update_v1(struct mmc_perf *perf, u32 qd, enum mmc_perf_op op, enum mmc_perf_entry entry)
{
	struct mmc_perf_stat_v1 *stat = &perf->stat_v1;
	enum policy_res res = R_OK;
	unsigned long flags;
	...
	switch (entry) {
	case MMC_PERF_ENTRY_QUEUED:
		__update_v1_queued(stat, qd);
		break;
	case MMC_PERF_ENTRY_RESET:
...
struct __sysfs_attr {
	...
	ssize_t (*show)(struct mmc_perf_stat_v1 *stat, char *buf);
	int (*store)(struct mmc_perf_stat_v1 *stat, u32 value);
};

#define __SYSFS_NODE(_name)							\
static ssize_t __sysfs_##_name##_show(struct mmc_perf_stat_v1 *stat,		\
						   char *buf)			\
{										\
	return snprintf(buf, PAGE_SIZE, "%u\n", stat->th_##_name);		\
};										\
static int __sysfs_##_name##_store(struct mmc_perf_stat_v1 *stat, u32 value)	\
{										\
...
__SYSFS_NODE(dwell_in_high);
__SYSFS_NODE(reach_up_to_high);
__SYSFS_NODE(duration);
__SYSFS_NODE(detect_dwell);
__SYSFS_NODE(detect_rare);
__SYSFS_NODE(count);
__SYSFS_NODE(reset_in_ms);
...
{
	struct mmc_perf_stat_v1 *stat = container_of(kobj, struct mmc_perf_stat_v1, sysfs_kobj);
...
static int __sysfs_init(struct mmc_perf_stat_v1 *stat)
{
	int error;

	/* create a path of /sys/kernel/mmc_perf_x */
	kobject_init(&stat->sysfs_kobj, &__sysfs_ktype);
	error = kobject_add(&stat->sysfs_kobj, kernel_kobj, "mmc_perf_v1_%c", (char)('0'));
	if (error) {
...
static inline void __sysfs_exit(struct mmc_perf_stat_v1 *stat)
{
	kobject_put(&stat->sysfs_kobj);
}

static void __reset_timer(unsigned long arg)
{
	struct mmc_perf *perf= (struct mmc_perf *)arg;
	struct mmc_perf_stat_v1 *stat = &perf->stat_v1;
	ktime_t time = ktime_get();
	bool transit = false;
	unsigned long flags;
	...
	spin_lock_irqsave(&stat->lock, flags);
	if (stat->hat_state >= HAT_REACH && stat->hat_state <= HAT_PRE_RARE) {
		stat->hat_state = HAT_DROP;
		stat->last_hat_transit_time = time;
		stat->last_drop_time = time;
		transit = true;
	...
	if (stat->freq_state >= FREQ_REACH && stat->freq_state <= FREQ_PRE_RARE) {
		stat->freq_state = FREQ_DROP;
		stat->last_freq_transit_time = time;
		transit = true;
	}
	...
	trace_mmc_perf_update_v1("reset", stat->count,
...
int mmc_perf_init_v1(struct mmc_perf *perf)
{
	...
	/* register update stat */
	perf->update[__UPDATE_V1] = __update_v1;

	/* register controls */
	perf->ctrl[__CTRL_REQ_DVFS] = __ctrl_dvfs;
	perf->ctrl[__CTRL_REQ_AFFINITY] = __ctrl_affinity;

	/* thresholds for stats */
	stat->th_qd_max = 15;
	stat->th_qd_min = 2;
	stat->th_dwell_in_high = 300;
	stat->th_reach_up_to_high = 50;

	stat->th_duration = 1;
	stat->th_count = 5;
	stat->th_detect_dwell = 3;
	stat->th_detect_rare = 12;
	stat->th_reset_in_ms = 12;

	/* init stats */
	stat->start_count_time = -1LL;

	for (index = 0; index < perf->perf_policy_num; index++) {
		con[index] = perf->policy[index].con;
	}

	/* enable bits */
	stat->policy_bits = BIT(perf->perf_policy_num - 1);
	for (index = 0; index < __POLICY_MAX; index++) {
		if (!(BIT(index) & stat->policy_bits))
			continue;
		stat->req_bits[index] = con[index];
	}
	...
	/* reset timer */
	setup_timer(&stat->reset_timer, __reset_timer, (unsigned long)perf);
