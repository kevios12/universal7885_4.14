...
static inline bool __is_data_cmd(struct mmc_request *mrq)
{
	struct mmc_cmdq_req *cmdq_req = mrq->cmdq_req;

	return ((cmdq_req && !(cmdq_req->cmdq_req_flags & DCMD) &&
		cmdq_req->data.blocks) ? true : false);
}
...
busy_wait:
...
#endif
	cmdq_req = mrq->cmdq_req;
	data_cmd = __is_data_cmd(mrq);
	...
	if (data_cmd) {
		...
		op = (cmdq_req->cmdq_req_flags & DIR) ?
		      MMC_PERF_OP_R : MMC_PERF_OP_W;
		mmc_perf_update(cq_host->perf, qd, op, mrq->cmdq_req->data.blocks * 512);
	} else if (mrq->cmd->opcode == MMC_SWITCH &&
		   ((mrq->cmd->arg & 0xFFFF00) >> 16) == EXT_CSD_FLUSH_CACHE) {
		mmc_perf_update(cq_host->perf, 0, MMC_PERF_OP_S, 0);
	}
	      ...
	      bool dma64, struct device *dev)
{
	...
	tcc_miss_period = 0;
#endif
	mmc_perf_init(&cq_host->perf, cq_host, dev);
